...
class Connection : public boost::enable_shared_from_this<Connection>
{
 public:
  ...
  virtual Error ChangeTrack2(const uint64_t recordingtoken, const uint64_t recordingjobtoken, const std::string& description, const bool fixedfiles, const bool digitalsigning, const bool encrypt, const uint32_t flushfrequency, const std::vector<uint64_t>& files, const std::string& mediauri, const std::string& username, const std::string& password, const std::vector<std::string>& receiverparameters, const std::vector<std::string>& sourceparameters, const std::vector<std::string>& objectdetectorsourceparameters) = 0;
