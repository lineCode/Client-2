...
boost::shared_future<server::ServerResponse> DummyEventService::Request(bool authenticated, evhttp_cmd_type requesttype, const std::string& localaddress, const uint16_t port, const std::string& remoteaddress, const std::string& path, const std::string& parameters, const std::vector<char>& content, const std::map< std::string, std::vector<char> >& mtomdata)
{
  ...
  std::string response;
  if (doc.select_node("/*[local-name()='Envelope']/*[local-name()='Body']/*[local-name()='GetEventProperties']"))
  {
    response = std::string("<media:GetEventPropertiesResponse>") + ToXml(std::string("TopicNamespaceLocation"), geteventpropertiestopicnamespacelocation_) + onvif::ToXml(std::string("FixedTopicSet"), boost::optional<bool>(geteventpropertiesfixedtopicset_)) + geteventpropertiestopicset_.ToXml(std::string("TopicSet")) + ToXml(std::string("TopicExpressionDialect"), geteventpropertiestopicexpressiondialect_) + ToXml(std::string("MessageContentFilterDialect"), geteventpropertiesmessagecontentfilterdialect_) + ToXml(std::string("ProducerPropertiesFilterDialect"), geteventpropertiesproducerpropertiesfilterdialect_) + ToXml(std::string("MessageContentSchemaLocation "), geteventpropertiesmessagecontentschemalocation_) + std::string("</media:GetEventPropertiesResponse>");
  ...
  else
  {
    ...
    return boost::make_ready_future(server::ServerResponse(server::HTTPSTATUSCODE::BADREQUEST, std::string("Error 400 Bad Request")));
  }
