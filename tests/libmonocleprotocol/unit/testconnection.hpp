...
class TestConnection : public server::Connection
{
 public:
  ...
  virtual Error ChangeRecording(const uint64_t token, const std::string& sourceid, const std::string& name, const std::string& location, const std::string& description, const std::string& address, const std::string& content, const uint64_t retentiontime, const bool adaptivestreaming) override;
  ...
  virtual std::pair< Error, std::vector<OBJECTTRACKSTATISTICS> > GetObjectTrackStatistics(const uint64_t recordingtoken, const uint32_t trackid, const uint64_t starttime, const uint64_t endtime, const uint64_t interval) override;
