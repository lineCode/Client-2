...
class TestConnection : public server::Connection
{
 public:
  ...
  virtual std::pair<Error, uint64_t> CreateFindObject(const uint64_t recordingtoken, const uint32_t tracktoken, const uint64_t starttime, const uint64_t endtime, const uint32_t minimumduration, const float x, const float y, const float width, const float height) override;
