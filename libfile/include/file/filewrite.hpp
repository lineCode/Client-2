...
class FileWrite
{
 public:
  ...
  int WriteObjectDetectorFrame(const uint64_t deviceindex, const uint64_t recordingindex, const uint64_t trackindex, const uint64_t codecindex, const uint8_t* data, const uint64_t size, const uint64_t time, const file::ObjectDetectorFrameType objectdetectorframetype, const std::vector<unsigned char>& signature);
 ...
 private:
  ...
  std::map<FRAMEINDEX, std::vector< std::unique_ptr<OBJECTDETECTORFRAMEHEADER> > > objectdetectorframeheaders_;
