// managerecordingjobsourceonvifdiscoverytree.cpp
//

///// Includes /////

#include "monocleclient/managerecordingjobsourceonvifdiscoverytree.h"
...
ManageRecordingJobSourceONVIFDiscoveryTree::ManageRecordingJobSourceONVIFDiscoveryTree(QWidget* parent) :
  QTreeWidget(parent),
  device_(nullptr),
  showipv4_(true),
  showipv6_(true),
  showlinklocal_(true)
{
  connect(this, &QTreeWidget::itemCollapsed, this, &ManageRecordingJobSourceONVIFDiscoveryTree::ItemCollapsed);
  connect(this, &QTreeWidget::itemExpanded, this, &ManageRecordingJobSourceONVIFDiscoveryTree::ItemExpanded);
  
  startTimer(std::chrono::milliseconds(60));
}

ManageRecordingJobSourceONVIFDiscoveryTree::~ManageRecordingJobSourceONVIFDiscoveryTree()
{
  connection_.Close();
...
void ManageRecordingJobSourceONVIFDiscoveryTree::Init(const boost::shared_ptr<Device>& device, const std::string& username, const std::string& password)
{
  device_ = device;
  username_ = username;
  password_ = password;

  connect(device_.get(), &Connection::SignalDiscoveryHello, this, &ManageRecordingJobSourceONVIFDiscoveryTree::DiscoveryHello);
...
void ManageRecordingJobSourceONVIFDiscoveryTree::SetTextFilter(const QString& textfilter)
{
  textfilter_ = textfilter;
  Filter();
}

void ManageRecordingJobSourceONVIFDiscoveryTree::ShowIpv4(const bool showipv4)
{
  showipv4_ = showipv4;
  Filter();
}

void ManageRecordingJobSourceONVIFDiscoveryTree::ShowIpv6(const bool showipv6)
{
  showipv6_ = showipv6;
  Filter();
}

void ManageRecordingJobSourceONVIFDiscoveryTree::ShowLinkLocal(const bool showlinklocal)
{
  showlinklocal_ = showlinklocal;
  Filter();
}

void ManageRecordingJobSourceONVIFDiscoveryTree::Refresh()
{
  clear();
  connection_.Close();
...
void ManageRecordingJobSourceONVIFDiscoveryTree::SetUserPass(const std::string& username, const std::string& password)
{
  username_ = username;
  password_ = password;
...
void ManageRecordingJobSourceONVIFDiscoveryTree::timerEvent(QTimerEvent* event)
{
  for (int i = 0; i < topLevelItemCount(); ++i)
  {
...
void ManageRecordingJobSourceONVIFDiscoveryTree::Filter()
{
  for (int i = 0; i < topLevelItemCount(); ++i)
  {
...
void ManageRecordingJobSourceONVIFDiscoveryTree::Filter(QTreeWidgetItem* item)
{
  if (textfilter_.size())
  {
...
bool ManageRecordingJobSourceONVIFDiscoveryTree::ChildrenContainsTextFilter(QTreeWidgetItem* item)
{
...
void ManageRecordingJobSourceONVIFDiscoveryTree::ItemCollapsed(QTreeWidgetItem* item)
{
  static_cast<ManageRecordingJobSourceONVIFDiscoveryTreeItem*>(item)->Collapsed();

}

void ManageRecordingJobSourceONVIFDiscoveryTree::ItemExpanded(QTreeWidgetItem* item)
{
  static_cast<ManageRecordingJobSourceONVIFDiscoveryTreeItem*>(item)->Expanded();

}

void ManageRecordingJobSourceONVIFDiscoveryTree::DiscoveryHello(const std::vector<std::string>& addresses, const std::vector<std::string>& scopes)
{
