      ...
      authenticate_ = Authenticate(username_.toStdString(), clientnonce, monocle::AuthenticateDigest(username_.toStdString(), password_.toStdString(), getauthenticationnonceresponse.authenticatenonce_, clientnonce), [this](const std::chrono::steady_clock::duration latency, const monocle::client::AUTHENTICATERESPONSE& authenticateresponse)
      {
        ...
        subscribe_ = Connection::Subscribe([this](const std::chrono::steady_clock::duration latency, const monocle::client::SUBSCRIBERESPONSE& subscriberesponse)
        {
          ...
          for (const monocle::RECORDING& r : subscriberesponse.recordings_)
          {
            ...
            {
              std::vector<DATASNAPSHOT> totaltrackdata;
              totaltrackdata.reserve(t.totaltrackdata_.size());
              for (const std::pair<uint64_t, uint64_t>& t : t.totaltrackdata_)
              {
                totaltrackdata.push_back(DATASNAPSHOT(t.first, t.second));
              ...
              {
                emit SignalRecordingTrackAdded(recording, recording->AddTrack(t.id_, t.token_, t.tracktype_, t.description_, t.fixedfiles_, t.digitalsignature_, t.encrypt_, t.flushfrequency_, t.files_, t.indices_, t.codecindices_, totaltrackdata));
              ...
              else
              {
                if (((*i)->GetToken().toStdString() != t.token_) || ((*i)->GetTrackType() != t.tracktype_) || ((*i)->GetDescription().toStdString() != t.description_) || ((*i)->GetFixedFiles() != t.fixedfiles_) || ((*i)->GetDigitalSigning() != t.digitalsignature_) || ((*i)->GetEncrypt() != t.encrypt_) || ((*i)->GetFlushFrequency() != t.flushfrequency_) || ((*i)->GetIndices() != t.indices_))
                {
                  ...
                  (*i)->SetTotalTrackData(totaltrackdata);
...
bool Device::SupportsAdaptiveStreaming() const
{
  if (version_ < utility::Version(1, 14, 0))
  {
...
void Device::SlotGuiOrderChanged(const std::vector<GUIORDER>& recordingsorder, const std::vector<GUIORDER>& mapsorder)
{
  for (const GUIORDER& recordingorder : recordingsorder)
  {
    std::vector< QSharedPointer<client::Recording> >::iterator r = std::find_if(recordings_.begin(), recordings_.end(), [&recordingorder](const QSharedPointer<client::Recording>& recording) { return (recording->GetToken() == recordingorder.token_); });
    if (r == recordings_.end())
    {
      LOG_GUI_WARNING_SOURCE(boost::static_pointer_cast<Device>(shared_from_this()), QString("Unable to find recording: ") + QString::number(recordingorder.token_));
      continue;
    }
    (*r)->SetGuiOrder(recordingorder.order_);
  }

  for (const GUIORDER& maporder : mapsorder)
  {
    std::vector< QSharedPointer<client::Map> >::iterator m = std::find_if(maps_.begin(), maps_.end(), [&maporder](const QSharedPointer<client::Map>& map) { return (map->GetToken() == maporder.token_); });
    if (m == maps_.end())
    {
      LOG_GUI_WARNING_SOURCE(boost::static_pointer_cast<Device>(shared_from_this()), QString("Unable to find map: ") + QString::number(maporder.token_));
      continue;
    }
    (*m)->SetGuiOrder(maporder.order_);
  }
...
void Device::SlotRecordingAdded(const uint64_t token, const std::string& sourceid, const std::string& name, const std::string& location, const std::string& description, const std::string& address, const std::string& content, const uint64_t retentiontime, const bool adaptivestreaming, const boost::optional<uint64_t>& activejob)
{
  if (state_ != DEVICESTATE::SUBSCRIBED)
  {
  ...
  if (r == recordings_.end())
  {
    QSharedPointer<client::Recording> recording = QSharedPointer<client::Recording>::create(boost::static_pointer_cast<Device>(shared_from_this()), token, QString::fromStdString(sourceid), QString::fromStdString(name), QString::fromStdString(location), QString::fromStdString(description), QString::fromStdString(address), QString::fromStdString(content), retentiontime, adaptivestreaming, activejob, 0);
    recordings_.push_back(recording);
    emit SignalRecordingAdded(recording);
  }
  else
  {
    if (((*r)->GetSourceId().toStdString() != sourceid) || ((*r)->GetName().toStdString() != name) || ((*r)->GetLocation().toStdString() != location) || ((*r)->GetDescription().toStdString() != description) || ((*r)->GetAddress().toStdString() != address) || ((*r)->GetContent().toStdString() != content) || ((*r)->GetRetentionTime() != retentiontime) || ((*r)->GetAdaptiveStreaming() != adaptivestreaming))
    {
      ...
      (*r)->SetRetentionTime(retentiontime);
      (*r)->SetAdaptiveStreaming(adaptivestreaming);
      emit SignalRecordingChanged(*r);
    }
  }
}

void Device::SlotRecordingChanged(const uint64_t token, const std::string& sourceid, const std::string& name, const std::string& location, const std::string& description, const std::string& address, const std::string& content, const uint64_t retentiontime, const bool adaptivestreaming, const boost::optional<uint64_t>& activejob)
{
  if (state_ != DEVICESTATE::SUBSCRIBED)
  {
  ...
  (*r)->Set(QString::fromStdString(sourceid), QString::fromStdString(name), QString::fromStdString(location), QString::fromStdString(description), QString::fromStdString(address), QString::fromStdString(content), retentiontime, adaptivestreaming, activejob);
  emit SignalRecordingChanged(*r);
}
...
void Device::SlotRecordingStatistics(const uint64_t time, const std::vector<monocle::RECORDINGSTATISTICS>& recordingsstatistics)
{
  for (const monocle::RECORDINGSTATISTICS& recordingstatistics : recordingsstatistics)
  {
    ...
    for (const monocle::TRACKSTATISTICS& trackstatistics : recordingstatistics.tracksstatistics_)
    {
      ...
      track->AddTotalTrackData(DATASNAPSHOT(time, trackstatistics.trackdatareceived_));
    }
...
void Device::SlotTrackAdded(const uint64_t recordingtoken, const uint32_t id, const std::string& token, const monocle::TrackType tracktype, const std::string& description, const bool fixedfiles, const bool digitalsigning, const bool encrypt, const uint32_t flushfrequency, const std::vector<uint64_t>& filetokens, const std::vector<monocle::CODECINDEX>& codecindices, const std::vector<DATASNAPSHOT>& totaltrackdata)
{
  if (state_ != DEVICESTATE::SUBSCRIBED)
  {
  ...
  {
    const QSharedPointer<client::RecordingTrack> track = (*recording)->AddTrack(id, token, tracktype, description, fixedfiles, digitalsigning, encrypt, flushfrequency, filetokens, std::vector<monocle::INDEX>(), codecindices, totaltrackdata);
    emit SignalRecordingTrackAdded(*recording, track);
  }
}

void Device::SlotTrackChanged(const uint64_t recordingtoken, const uint32_t id, const std::string& token, const monocle::TrackType tracktype, const std::string& description, const bool fixedfiles, const bool digitalsigning, const bool encrypt, const uint32_t flushfrequency, const std::vector<uint64_t>& filetokens, const std::vector<monocle::CODECINDEX>& codecindices, const std::vector<DATASNAPSHOT>& totaltrackdata)
{
  if (state_ != DEVICESTATE::SUBSCRIBED)
  {
