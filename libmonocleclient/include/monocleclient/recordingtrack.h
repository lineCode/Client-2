...
class RecordingTrack : public QObject
{
 ...
 public:
  ...
  RecordingTrack(const uint32_t id, const QString& token, const monocle::TrackType tracktype, const QString& description, const bool fixedfiles, const bool digitalsigning, const bool encrypt, const uint32_t flushfrequency, const std::vector<uint64_t>& files, const std::vector< std::pair<uint64_t, uint64_t> >& indices, const std::vector<monocle::CODECINDEX>& codecindices, const std::vector<DATASNAPSHOT>& totaltrackdata);
  ~RecordingTrack();

  void ChangeTrack(const QString& token, const monocle::TrackType tracktype, const QString& description, const bool fixedfiles, const bool digitalsigning, const bool encrypt, const uint32_t flushfrequency, const std::vector<uint64_t>& files, const std::vector<monocle::CODECINDEX>& codecindices, const std::vector<DATASNAPSHOT>& totaltrackdata);
  ...
  void AddTotalTrackData(const DATASNAPSHOT& totaltrackdata);
  void SetTotalTrackData(const std::vector<DATASNAPSHOT>& totaltrackdata);
  inline const boost::circular_buffer<DATASNAPSHOT>& GetTotalTrackData() const { return totaltrackdata_; }
 ...
 private:
  ...
  boost::circular_buffer<DATASNAPSHOT> totaltrackdata_; // <time, bytes>
